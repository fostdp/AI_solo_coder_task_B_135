...
"""照度计算模块 - 从optical_params.json加载建筑和材质参数"""
...
class BuildingGeometry:
    """建筑几何参数 - 从optical_params.json building节加载默认值"""
    ...
    def __post_init__(self):
        if not self.windows:
            cfg = self._get_config().get("building", {})
            default_windows = cfg.get("default_windows", None)
            if default_windows:
    ...
    @classmethod
    def from_config(cls, config: Optional[Dict[str, Any]] = None) -> 'BuildingGeometry':
        cfg = config if config is not None else cls._get_config().get("building", {})
        return cls(
            length=cfg.get("length", 20.0),
            width=cfg.get("width", 20.0),
            height=cfg.get("height", 12.0),
            wall_thickness=cfg.get("wall_thickness", 0.5),
            windows=cfg.get("default_windows", []),
        )
...
class LightingCalculator:
    ...
    def _create_materials(self) -> Dict[str, Material]:
        cfg = self._get_config().get("materials", {})
        materials: Dict[str, Material] = {}
        for key, mat_cfg in cfg.items():
            materials[key] = Material.from_config(key, mat_cfg)
        if not materials:
            materials = {
                'floor': Material(name='wooden_floor', albedo=np.array([0.6, 0.5, 0.4]), reflectivity=0.1, roughness=0.8),
                'wall': Material(name='rammed_earth', albedo=np.array([0.7, 0.65, 0.55]), reflectivity=0.05, roughness=0.9),
                'roof': Material(name='ceramic_tile', albedo=np.array([0.5, 0.3, 0.2]), reflectivity=0.05, roughness=0.95),
                'column': Material(name='wooden_column', albedo=np.array([0.4, 0.25, 0.15]), reflectivity=0.1, roughness=0.7),
                'window': Material(name='paper_window', albedo=np.array([0.9, 0.9, 0.85]), transmittance=0.7, reflectivity=0.1, roughness=0.3),
                'ground': Material(name='soil', albedo=np.array([0.3, 0.25, 0.2]), reflectivity=0.1, roughness=0.95),
            }
        return materials
    ...
    def _build_scene(self):
        ...
        window_mat_cfg = self._get_config().get("materials", {}).get("window", None)

        for window_config in b.windows:
            if window_mat_cfg:
                window_material = Material.from_config("window", window_mat_cfg)
                window_material = Material(
                    name=window_material.name,
                    albedo=window_material.albedo,
                    transparency=window_material.transparency,
                    reflectivity=window_material.reflectivity,
                    roughness=window_material.roughness,
                    transmittance=window_config.get('transmittance', window_material.transmittance),
                )
            else:
                window_material = Material(
                    name='paper_window',
    ...
    def _add_columns(self):
        b = self.building
        building_cfg = self._get_config().get("building", {})
        column_inset = building_cfg.get("column_inset", 1.5)
        column_size = building_cfg.get("column_size", 0.4)

        half_len = b.length / 2 - column_inset
        half_wid = b.width / 2 - column_inset
    ...
    def _generate_grid_points(self) -> Tuple[np.ndarray, Tuple[int, int, int]]:
        ...
        grid_cfg = self._get_config().get("grid", {})
        z_levels_ratio = grid_cfg.get("z_levels_ratio", 2)
        z_min_levels = grid_cfg.get("z_min_levels", 3)
        z_bottom_offset = grid_cfg.get("z_bottom_offset", 0.5)
        z_top_offset = grid_cfg.get("z_top_offset", 0.5)

        nx = ny = self.grid_resolution
        nz = max(z_min_levels, self.grid_resolution // z_levels_ratio)

        x = np.linspace(-half_len, half_len, nx)
        y = np.linspace(-half_wid, half_wid, ny)
        z = np.linspace(z_bottom_offset, b.height - z_top_offset, nz)
    ...
    def calculate_illuminance(
        ...
        night_illuminance = self._get_config().get("night_illuminance", 5.0)

        if altitude <= 0:
            points, grid_size = self._generate_grid_points()
            illuminance_matrix = np.full(grid_size, night_illuminance)
            return IlluminanceResult(
                timestamp=dt,
                grid_size=grid_size,
                illuminance_matrix=illuminance_matrix,
                max_illuminance=night_illuminance,
                min_illuminance=night_illuminance,
                avg_illuminance=night_illuminance,
                uniformity=1.0,
                solar_altitude=0.0,
                solar_azimuth=0.0
            )
