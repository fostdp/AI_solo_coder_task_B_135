...
class InfluxDBManager:
    ...
    def _initialize(self):
        self.client = InfluxDBClient(
            url=shared_settings.influxdb_url,
            token=shared_settings.influxdb_token,
            org=shared_settings.influxdb_org,
            timeout=30000,
        )
        self.write_api = self.client.write_api(write_options=SYNCHRONOUS)
        self.query_api = self.client.query_api()
        self.bucket = shared_settings.influxdb_bucket
        self.org = shared_settings.influxdb_org
    ...
    def write_sensor_data(self, data_points: List[Dict]) -> bool:
        try:
            ...
            logger.info(f"Wrote {len(points)} sensor points")
            return True
    ...
    def query_sensor_data(
        ...
        try:
            ...
            hall_id_safe = hall_id.replace('"', '\\"')
            flux_query = f'''
                from(bucket: "{self.bucket}")
                    |> range(start: {start_time.isoformat()}, stop: {end_time.isoformat()})
                    |> filter(fn: (r) => r["_measurement"] == "sensor_measurements")
                    |> filter(fn: (r) => r["hall_id"] == "{hall_id_safe}")
            '''
            if location:
                location_safe = location.replace('"', '\\"')
                flux_query += f'|> filter(fn: (r) => r["location"] == "{location_safe}")\n'
            if fields:
    ...
    def query_alert_history(
        ...
        try:
            ...
            if hall_id:
                hall_id_safe = hall_id.replace('"', '\\"')
                flux_query += f'|> filter(fn: (r) => r["hall_id"] == "{hall_id_safe}")\n'
    ...
    def calculate_daily_sunshine_hours(self, hall_id: str, date: datetime) -> float:
        try:
