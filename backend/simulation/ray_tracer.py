...
class RayTracer:
    def __init__(self, max_depth: int = 3, bias: float = 0.001):
    ...
    def generate_hemisphere_samples(self, normal: np.ndarray, num_samples: int = 16) -> List[np.ndarray]:
        samples = []
        for i in range(num_samples):
            u1 = (i + 0.5) / num_samples
            ...
            tangent = np.array([1, 0, 0]) if abs(normal[0]) < 0.9 else np.array([0, 1, 0])
